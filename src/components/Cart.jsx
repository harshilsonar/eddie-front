...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get('https://eddie-server-1.onrender.com/cart');
  ...
  const updateQuantity = async (id, type) => {
    try {
      ...
      await axios.patch(`https://eddie-server-1.onrender.com/cart/${id}`, {
  ...
  const removeItem = async (id) => {
    try {
      await axios.delete(`https://eddie-server-1.onrender.com/cart/${id}`);
