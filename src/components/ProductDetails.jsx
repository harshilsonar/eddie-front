...
const { type, id } = useParams();
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://eddie-server-1.onrender.com/${type}/${id}`)
      .then((res) => {
  ...
  const handleColorChange = (swatch) => {
    ...
    axios.get(`https://eddie-server-1.onrender.com/${type}/${id}`)
      .then((res) => {
        if (res.data.images) {
  ...
  const addToCart = () => {
    ...
    axios
      .get("https://eddie-server-1.onrender.com/cart")
      .then((res) => {
        ...
        if (existingItem) {
          ...
          return axios.patch(`https://eddie-server-1.onrender.com/cart/${existingItem.id}`, {
            quantity: updatedQuantity,
            totalPrice: updatedQuantity * existingItem.price,
          });
        } else {
          return axios.post("https://eddie-server-1.onrender.com/cart", cartItem);
        }
      })
      .then(() => {
